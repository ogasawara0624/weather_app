...
app.get("/", (req, res) => {
  res.send("API is working. Use /engineer-positions?year=YYYY&month=MM");
});
...
app.get("/api/current", async (req: Request, res: Response) => {
  ...
  try {
    // ① OpenCage で緯度経度取得
    const geoRes = await fetch(`https://api.opencagedata.com/geocode/v1/json?q=${encodeURIComponent(city)}&key=${GEOCODE_API_KEY}&language=ja`);
    ...
    // ② WeatherAPI で天気取得
    const weatherRes = await fetch(`https://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${lat},${lng}&lang=ja`);
...
app.get("/api/hourly", async (req: Request, res: Response) => {
  ...
  try {
    // OpenCageで緯度経度取得
    const geoRes = await fetch(`https://api.opencagedata.com/geocode/v1/json?q=${encodeURIComponent(city)}&key=${GEOCODE_API_KEY}&language=ja`, {signal: controller.signal});
    clearTimeout(timeout);
    ...
    const weatherRes = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${lat},${lng}&days=1&lang=ja`);
    if (!weatherRes.ok) throw new Error("天気APIエラー");
...
app.listen(6810, () => {
  console.log("✅ Server running at http://localhost:6810");
});